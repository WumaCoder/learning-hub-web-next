<template>
  ...
  <div class="p-grid">
    <div class="p-col-9">
      ...
      <p-panel v-if="problem.data.options" class="p-mt-2" header="Options">
        <div
        ...
        <p-button @click="onClickSubmit" :disabled="runState.padding">
          ...
          />
          {{ runState.padding ? "Pinding..." : "Submit !" }}</p-button
        >
        <transition-group name="p-message" tag="div">
          <p-message
            v-if="runState.show"
            :severity="runState.result === 'OK' ? 'success' : 'error'"
            >{{ runState.result }}</p-message
          >
        </transition-group>
      </p-panel>
...
import { getJob, judgeSingle } from "api/judger";
...
export default {
  ...
  setup() {
    ...
    const onClickSubmit = async () => {
      ...
      const body = await judgeSingle(model);
